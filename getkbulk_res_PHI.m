function[elmat,rmat]=getkbulk_res_PHI(iel,IntOrder,u,phi)
...
%    end  
    ...
    kpp=kpp+(((2*SED0+Gc/Lc)*N*N')+(Gc*Lc*BPhi'*BPhi))*W(kk)*det(J0);
    rphi=rphi+((dgphi*N*SED0)+Gc*(1/Lc*N*phix+Lc*BPhi'*delPHI))*W(kk)*det(J0);
    
end
sctrPhiloc=[9:12];
elmat(sctrPhiloc,sctrPhiloc)=kpp;
rmat(sctrPhiloc,1)=rphi;